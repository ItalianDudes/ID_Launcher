...
public final class ControllerSceneTabDespicableCards {
    ...
    // Constants
    private static final File INSTALLATION_DIR = new File(new File(Defs.JAR_POSITION).getParent() + File.separator + "DespicableCards");
    private static final String APP_NAME = "DespicableCards";
            ...
            try {
                if (!INSTALLATION_DIR.exists()) {
                    if (!INSTALLATION_DIR.mkdir()) {
                        Platform.runLater(ControllerSceneTabDespicableCards.this::resetController);
    ...
    private void startVersion() {
        ...
        JFXDefs.startServiceTask(() -> {
            ...
            try {
                ...
                Class<?> despicableCardsMainClass = classLoader.loadClass("it.italiandudes.despicable_cards.DespicableCards");
                try {
                    despicableCardsMainClass.getMethod("launcherMain", ClassLoader.class, String[].class).invoke(null, classLoader, new String[]{});
                ...
                switch (behaviour) {
                    case CLOSE_ON_LAUNCH:
                        Logger.log("Starting DespicableCards, launcher closing...");
                        Platform.runLater(() -> Client.getStage().hide());
                        break;

                    case MINIMIZE:
                        Logger.log("Starting DespicableCards, minimizing launcher...");
                        Platform.runLater(() -> Client.getStage().setIconified(true));
                        break;

                    case STAY_OPEN:
                        Logger.log("Starting DespicableCards, staying open...");
                        break;

                    case HIDE_UNTIL_CLOSE:
                        Logger.log("Starting DespicableCards, hiding launcher...");
                        Platform.runLater(() -> Client.getStage().hide());
                        break;
                }
                boolean behaviourCompatible = true;
                try {
                    despicableCardsMainClass.getMethod("launcherLockUntilAppClose").invoke(null); // Blocker Method
                } catch (NoSuchMethodException e) {
                ...
                switch (behaviour) {
                    ...
                    case HIDE_UNTIL_CLOSE:
                        Logger.log("DespicableCards closed, reshowing launcher...");
